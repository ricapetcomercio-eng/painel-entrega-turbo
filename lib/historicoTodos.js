...
async function registrarHistoricoTodos(redis, pedidos) {
  ...
  // de pedidos não sabe nada sobre devolução. Sem isso, um pedido já
  // marcado como devolvido perderia essa marcação sempre que fosse
  // recoletado/reprocessado (ex: backfill manual sobre o mesmo período).
  const idsUnicos = pedidos.map((p) => `${p.marketplace || 'mercado_livre'}:${p.order_id}`);
  const existentes = {};
  const TAMANHO_LOTE_LEITURA = 500;
  for (let i = 0; i < idsUnicos.length; i += TAMANHO_LOTE_LEITURA) {
    const lote = idsUnicos.slice(i, i + TAMANHO_LOTE_LEITURA);
    const valores = await redis.hmget(HISTORICO_TODOS_HASH_KEY, ...lote);
    if (valores) Object.assign(existentes, valores);
  }
  ...
  for (const pedido of pedidos) {
    ...
    campos[idUnico] = {
      ...
      devolvido: typeof pedido.devolvido === 'boolean' ? pedido.devolvido : (anterior.devolvido || false),
      devolucao_claim_id: pedido.devolucao_claim_id || anterior.devolucao_claim_id || null,
      devolucao_status: pedido.devolucao_status || anterior.devolucao_status || null,
      devolucao_reason_id: pedido.devolucao_reason_id || anterior.devolucao_reason_id || null,
      itens: pedido.itens || [],
    };
