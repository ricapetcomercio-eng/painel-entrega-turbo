...
const { coletarTransferenciasEspecificas } = require('./omieExtrato');
...
// Dia da semana de uma chave 'AAAA-MM-DD' (0=domingo...6=sábado), sem
// depender de fuso — meio-dia UTC nunca cruza a virada do dia em nenhum
// fuso real, então dá pra tratar a chave como data de calendário pura.
function diaDaSemana(diaChave) {
  return new Date(`${diaChave}T12:00:00Z`).getUTCDay();
}

function ehFimDeSemanaChave(diaChave) {
  const dow = diaDaSemana(diaChave);
  return dow === 0 || dow === 6;
}

function proximoDiaChave(diaChave) {
  const d = new Date(`${diaChave}T12:00:00Z`);
  d.setUTCDate(d.getUTCDate() + 1);
  return d.toISOString().slice(0, 10);
}

// Um título com vencimento no passado só "dobra" pro dia de hoje se o
// atraso for causado só pelo fim de semana (sem expediente bancário pra
// pagar) — ou seja, `diaVencimento` cai num sábado/domingo E não existe
// nenhum dia útil entre ele e `diaHoje`. Decisão explícita do dono do
// projeto: atraso de verdade (vencido num dia útil e ainda não pago) NÃO
// aparece mais na tela — só o atraso "artificial" do fim de semana.
function atrasoEhSoDeFimDeSemana(diaVencimento, diaHoje) {
  if (!ehFimDeSemanaChave(diaVencimento)) return false;
  let cursor = proximoDiaChave(diaVencimento);
  while (cursor < diaHoje) {
    if (!ehFimDeSemanaChave(cursor)) return false;
    cursor = proximoDiaChave(cursor);
  }
  return true;
}
...
async function coletarContasPagar(conta) {
  ...
  for (let pagina = 1; pagina <= MAX_PAGINAS; pagina++) {
    ...
    for (const t of titulos) {
      ...
      let diaStr = dataOmieParaChave(t.data_vencimento);
      if (diaStr > futuroChave) continue; // fora do horizonte futuro, ignora
      // Título com vencimento no passado: só "dobra" pra hoje se o atraso
      // for causado SÓ pelo fim de semana (venceu sábado/domingo, sem
      // expediente bancário pra pagar) — decisão explícita do dono do
      // projeto. Atraso de verdade (venceu num dia útil e ainda não foi
      // pago) volta a ficar de fora da tela — dinheiro que devia ter saído
      // num dia útil e não saiu é tratado fora desta ferramenta, não
      // empurrado pro "hoje" indefinidamente.
      let venceuFimDeSemana = false;
      let diaVencimentoOriginal = null;
      if (diaStr < hojeChave) {
        if (!atrasoEhSoDeFimDeSemana(diaStr, hojeChave)) continue;
        venceuFimDeSemana = true;
        diaVencimentoOriginal = diaStr;
        diaStr = hojeChave;
      }
      ...
      atual.titulos.push({
        ...
        venceu_fim_de_semana: venceuFimDeSemana,
        data_vencimento_original: diaVencimentoOriginal,
      });
  ...
  // Duas transferências específicas entre contas (ex.: "Transf. ITAÚ
  // CORRENTE >> CARTÃO DE CRÉDITO - ITAÚ") pedidas pelo dono do projeto —
  // ver lib/omieExtrato.js. Precisa rodar DEPOIS de resolverNomesFornecedores
  // (que sobrescreve fornecedor_nome de TODO título sem
  // codigo_cliente_fornecedor pra null) — senão o rótulo da transferência
  // seria apagado. Falha engolida (não derruba a coleta inteira de Contas
  // a Pagar) — a API de Extrato é terreno novo/menos testado que
  // ListarContasPagar.
  try {
    const transferencias = await coletarTransferenciasEspecificas(conta);
    for (const tr of transferencias) {
      const atual = porDiaMap.get(tr.data) || { total: 0, qtd: 0, titulos: [] };
      atual.total += tr.valor;
      atual.qtd += 1;
      atual.titulos.push({
        valor: Math.round(tr.valor * 100) / 100,
        numero_documento: null,
        observacao: tr.descricao,
        status: tr.situacao,
        codigo_lancamento: tr.codigo_lancamento,
        codigo_cliente_fornecedor: null,
        fornecedor_nome: tr.rotulo,
        venceu_fim_de_semana: false,
        data_vencimento_original: null,
        transferencia: true,
      });
      porDiaMap.set(tr.data, atual);
      totalAPagar += tr.valor;
      totalTitulos += 1;
    }
  } catch (err) {
    // segue sem as transferências nesta coleta — Contas a Pagar continua
    // funcionando normalmente com os títulos já buscados acima.
  }
 ...
 * data_vencimento bate com `diaAlvo` ('AAAA-MM-DD'), ou caia dentro de
 * `diasJanela` dias antes/depois dele (default 0 = só o dia exato — útil
 * pra testar se um título "sumido" na verdade tem data_vencimento
 * ligeiramente diferente do que a Omie mostra como "vence hoje" na
 * Movimentação da Conta Corrente). Resolve o nome do fornecedor de cada
 * achado (poucos títulos por dia, então sem a trava de segurança usada em
 * resolverNomesFornecedores). Usado só por
 * /api/debug?tipo=omie-contas-pagar-dia pra achar títulos que somem da
 * Projeção Financeira sem explicação óbvia (ex.: um título fica de fora do
 * total mas não aparece aqui de jeito nenhum = nunca chegou nesta
 * paginação, problema de janela/volume; aparece aqui mas com status_titulo
 * inesperado = problema no filtro de status; só aparece com `diasJanela`
 * > 0 = data_vencimento diferente do que a Omie mostra como "vence hoje").
 * @param {'ricapet'|'thapets'} conta
 * @param {string} diaAlvo 'AAAA-MM-DD'
 * @param {number} [diasJanela] dias antes/depois de diaAlvo a incluir (default 0)
 */
async function listarTitulosPorDia(conta, diaAlvo, diasJanela) {
  const { appKey, appSecret } = getOmieConfig(conta);
  const raio = Number(diasJanela) > 0 ? Math.min(Number(diasJanela), 30) : 0;
  const diaAlvoMs = new Date(`${diaAlvo}T12:00:00-03:00`).getTime();
  const diaMinChave = raio ? dataFusoLoja(new Date(diaAlvoMs - raio * 24 * 60 * 60 * 1000)) : diaAlvo;
  const diaMaxChave = raio ? dataFusoLoja(new Date(diaAlvoMs + raio * 24 * 60 * 60 * 1000)) : diaAlvo;
  ...
  for (let pagina = 1; pagina <= MAX_PAGINAS; pagina++) {
    ...
    for (const t of titulos) {
      if (!t.data_vencimento) continue;
      const diaTitulo = dataOmieParaChave(t.data_vencimento);
      if (diaTitulo < diaMinChave || diaTitulo > diaMaxChave) continue;
      encontrados.push({
        ...
        data_vencimento_chave: diaTitulo,
        observacao: t.observacao || null,
      });
    }
  ...
  // Resolve nome do fornecedor pra cada achado (mesma API usada em
  // resolverNomeFornecedor) -- só pros poucos títulos de 1 dia, então dá
  // pra resolver todos sem trava de segurança. Facilita MUITO comparar
  // visualmente com o nome que aparece na Movimentação da Conta Corrente
  // do Omie, em vez de precisar decodificar codigo_cliente_fornecedor.
  const nomesPorCodigo = new Map();
  for (const item of encontrados) {
    const codigo = item.codigo_cliente_fornecedor;
    if (!codigo || nomesPorCodigo.has(codigo)) continue;
    nomesPorCodigo.set(codigo, await resolverNomeFornecedor(appKey, appSecret, codigo));
  }
  encontrados.forEach((item) => {
    item.fornecedor_nome = item.codigo_cliente_fornecedor ? (nomesPorCodigo.get(item.codigo_cliente_fornecedor) || null) : null;
  });

  encontrados.sort((a, b) => (a.data_vencimento_chave < b.data_vencimento_chave ? -1 : 1));
  return { dia: diaAlvo, janela_dias: raio, de: diaMinChave, ate: diaMaxChave, paginas_varridas: paginasVarridas, truncado, encontrados };
}
