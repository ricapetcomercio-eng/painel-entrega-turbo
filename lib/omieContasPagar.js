...
async function coletarContasPagar(conta) {
  ...
  for (let pagina = 1; pagina <= MAX_PAGINAS; pagina++) {
    ...
    for (const t of titulos) {
      ...
      // "quais contas compõem esse valor" ao clicar no dia. numero_documento/
      // observacao são os campos que a Omie retorna pra identificar o título
      // sem precisar de outra chamada (a API de Contas a Pagar não devolve
      // nome do fornecedor, só o código — resolver o nome exigiria uma
      // chamada extra por fornecedor, fora do escopo por ora).
      atual.titulos.push({
