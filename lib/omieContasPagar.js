...
async function coletarContasPagar(conta) {
  ...
  for (let pagina = 1; pagina <= MAX_PAGINAS; pagina++) {
    ...
    for (const t of titulos) {
      ...
      // Título ATRASADO (data_vencimento no passado, ainda não pago) NÃO é
      // descartado — dobra pra "hoje". Confirmado com dado real de
      // produção (endpoint omie-contas-pagar-dia): um título com
      // status_titulo "VENCE HOJE" tinha data_vencimento do dia ANTERIOR —
      // a própria Omie já bucketiza atrasado-e-não-pago junto com "hoje"
      // na Movimentação da Conta Corrente, então seguimos o mesmo critério
      // (em vez de simplesmente sumir do Fluxo de Caixa, que é pior:
      // dinheiro que já devia ter saído da conta precisa aparecer em
      // algum lugar, não desaparecer).
      const atrasado = diaStr < hojeChave;
      if (atrasado) diaStr = hojeChave;
      ...
      atual.titulos.push({
        ...
        atrasado,
        data_vencimento_original: atrasado ? dataOmieParaChave(t.data_vencimento) : null,
      });
