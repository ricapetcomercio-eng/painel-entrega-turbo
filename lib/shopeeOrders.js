...
function montarPedidoGenericoShopee(loja, pedido) {
  ...
  return {
    ...
    // Prazo real do vendedor pra despachar — pedido em response_optional_fields
    // (ver buscarDetalhesCompletos; testado sem isso e a Shopee não devolvia o
    // campo). Usado no painel da TV pra dar um cronômetro de verdade nesses
    // pedidos, igual o Flex tem o dele; sem isso, o card cai no modo "sem
    // prazo" (só mostra há quanto tempo está parado, sem countdown).
    prazo_entrega: pedido.ship_by_date ? new Date(pedido.ship_by_date * 1000).toISOString() : null,
    itens: (pedido.item_list || []).map((item) => ({
      titulo: item.item_name,
      quantidade: item.model_quantity_purchased,
      sku: item.model_sku || item.item_sku,
