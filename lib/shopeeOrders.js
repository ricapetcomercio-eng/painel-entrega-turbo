...
async function descobrirChannelIdTurbo(loja) {
  ...
  // Cacheia indefinidamente — canais raramente mudam. A tabela kv_simples
  // não tem expiração automática (diferente do Redis); se um dia precisar
  // forçar nova busca, é só apagar a linha "shopee_turbo_channel:<loja>".
  await kvSet(cacheKey, canalTurbo.logistics_channel_id);
