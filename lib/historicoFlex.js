 ...
 *
 * Inclui TAMBÉM qualquer pedido com categoria "aguardando" independente da
 * idade — sem isso, um pedido que ainda não foi coletado depois de 48h
 * (exatamente o caso que mais precisa de atenção) sumia do painel E parava
 * de ser reverificado contra a API do ML, porque saía da janela usada tanto
 * pra exibição quanto pra decidir o que reverificar. Confirmado com um
 * pedido real preso há mais de 2 dias, ainda "ready_to_ship" na Shopee/ML.
 */
async function listarRecentes(horasRetroativas = 48) {
  const db = getDb();
  const desde = Date.now() - horasRetroativas * 60 * 60 * 1000;
  const rs = await db.execute({
    sql: "SELECT * FROM historico_flex WHERE date_created_ts >= ? OR categoria = 'aguardando' ORDER BY date_created_ts ASC",
    args: [desde],
  });
