...
/**
 * Troca um "code" de autorização (fluxo OAuth completo, feito manualmente
 * uma vez pelo usuário) por access_token/refresh_token — usado quando o
 * escopo de permissões do app muda (ex: Publicidade) e o refresh_token
 * salvo foi emitido antes disso, então não carrega o escopo novo mesmo com
 * o app já configurado corretamente.
 */
async function trocarCodigoPorTokens(clientId, clientSecret, code, redirectUri) {
  ...
  if (!resp.ok) {
    const errText = await resp.text();
    throw new Error(`Falha ao trocar code por token ML (${resp.status}): ${errText}`);
  }
  return resp.json();
}
...
module.exports = { getMLAccessToken, CONTAS, getContaConfig, trocarCodigoPorTokens, salvarToken };
