...
async function registrarHistoricoTurboLive(pedidos) {
  ...
  for (const pedido of pedidos) {
    ...
    // Mesma proteção contra regressão de lib/historicoFlex.js: não deixa
    // uma categoria já resolvida (coletado/entregue/...) voltar pra
    // "aguardando" só porque o ciclo seguinte de descoberta/reverificação
    // ainda vê o pedido como pendente do lado da API — a bipagem no
    // galpão (api/marcar-coletado.js) é mais rápida que isso.
    await db.execute({
      sql: `INSERT INTO historico_turbo_live (
            ...
            ON CONFLICT(id_unico) DO UPDATE SET
              ...
              categoria = CASE
                WHEN historico_turbo_live.categoria IS NOT NULL AND historico_turbo_live.categoria != 'aguardando' AND excluded.categoria = 'aguardando'
                  THEN historico_turbo_live.categoria ELSE excluded.categoria END,
              status_pedido = excluded.status_pedido,
              resolvido_em = CASE
                WHEN historico_turbo_live.categoria IS NOT NULL AND historico_turbo_live.categoria != 'aguardando' AND excluded.categoria = 'aguardando'
                  THEN historico_turbo_live.resolvido_em ELSE excluded.resolvido_em END,
