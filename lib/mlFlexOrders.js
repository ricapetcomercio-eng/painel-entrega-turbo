...
// Prazo real do Flex: entrega prometida até as 21h do MESMO DIA (horário de
...
function montarPedidoFlex(conta, pedido, shipmentId, info) {
  ...
  const deadline = calcularDeadlineFlex(pedido.date_created);
...
async function reverificarStatusPedido(pedidoArmazenado) {
  ...
  const deadline = calcularDeadlineFlex(pedidoArmazenado.date_created);
  const deadlineMs = new Date(deadline).getTime();
  const fracaoConsumida = (Date.now() - dataVenda) / (deadlineMs - dataVenda);
...
module.exports = { coletarPedidosFlex, buscarPedidosPeriodo, verificarFlex, montarPedidoFlex, reverificarStatusPedido, calcularDeadlineFlex };
