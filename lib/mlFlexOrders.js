...
/**
 * Busca o prazo oficial calculado pelo próprio ML (GET /shipments/{id}/sla),
 * validado em 25/07/2026 contra um pedido Flex real (devolveu expected_date
 * e status "on_time" corretamente, não só pra Envios Agora). Preferimos esse
 * valor sobre o nosso cálculo aproximado (calcularDeadline) sempre que
 * disponível — é o prazo exato daquele pedido específico, não uma regra
 * genérica. Retorna null se o endpoint falhar, pra quem chamou cair no
 * cálculo próprio como plano B.
 */
async function buscarSlaShipment(conta, shipmentId) {
  try {
    const accessToken = await getMLAccessToken(conta);
    const sla = await mlFetch(`/shipments/${shipmentId}/sla`, accessToken);
    if (!sla || !sla.expected_date) return null;
    return { deadline: new Date(sla.expected_date).toISOString(), status: sla.status || null };
  } catch (err) {
    return null;
  }
}
...
async function verificarFlex(conta, shipmentId) {
  ...
  // cancelled também é estado final — confirmado com pedido real (comprador
  // cancelou). Sem isso, o pedido ficava contando atraso pra sempre mesmo
  // não havendo mais nada a coletar.
  else if (shipment.status === 'cancelled') categoria = 'cancelado';
  ...
  if (categoria !== 'aguardando') {
    ...
    // Só busca o SLA enquanto o pedido ainda está pendente — é o único caso
    // em que o deadline realmente importa (pedido já resolvido não mostra
    // contagem regressiva), então evitamos gastar chamada de API à toa.
    sla = await buscarSlaShipment(conta, shipmentId);
  }
  ...
  return {
    ...
    coletado_em,
    entregue_em,
    sla_deadline: sla ? sla.deadline : null,
  };
}
...
function montarPedidoFlex(conta, pedido, shipmentId, info) {
  ...
  const deadline = info.sla_deadline || calcularDeadline(info.tipo, pedido.date_created);
...
async function reverificarStatusPedido(pedidoArmazenado) {
  ...
  const deadline = info.sla_deadline || calcularDeadline(info.tipo, pedidoArmazenado.date_created);
  const deadlineMs = new Date(deadline).getTime();
  const fracaoConsumida = (Date.now() - dataVenda) / (deadlineMs - dataVenda);
