...
function montarPedidoGenerico(conta, pedido, shipmentId, detalhes) {
  ...
  return {
    ...
    itens: (pedido.order_items || []).map((oi) => ({
      ...
      // ⚠️ TODO: validar contra um pedido real — unit_price é o valor por
      // unidade já com desconto do item (confirmar se full_unit_price seria
      // mais correto pra "vendas" no dashboard de produtos).
      valor_unitario: typeof oi.unit_price === 'number' ? oi.unit_price : null,
    })),
  };
}
