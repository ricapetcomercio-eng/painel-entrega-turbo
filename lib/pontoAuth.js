...
// Sessão do painel administrativo (navegador) expira sozinha depois de
// 8h (um turno de trabalho) -- decisão explícita do dono do projeto:
// antes o token não tinha expiração nenhuma embutida, então uma aba
// fixada/restaurada pelo navegador continuava logada indefinidamente.
...
async function obterAdminSessao(token, db, pagina) {
  ...
  // Tokens gerados antes desta mudança não têm `emissao` -- tratados como
  // expirados (força um novo login) em vez de ficarem válidos pra sempre.
  if (!funcionario.emissao || Date.now() - funcionario.emissao > DURACAO_SESSAO_ADMIN_MS) {
