...
// Sessão do painel administrativo expira por INATIVIDADE (não por tempo
// fixo desde o login) -- decisão explícita do dono do projeto: 5min sem
// nenhuma atividade real (clique, tecla, scroll, mousemove) e a próxima
// validação recusa o token, mesmo com a aba aberta/fixada. `emissao` no
// payload do token passa a significar "última renovação", não "login" --
// assets/auth.js chama tipo=ponto-renovar-sessao (TIPOS_SESSAO_ADMIN, ver
// api/debug.js) periodicamente ENQUANTO detecta atividade recente, cada
// renovação chama gerarTokenPonto de novo (novo `emissao`, mesmo
// id/nome) -- puro HMAC, sem consulta a banco, custo de CPU desprezível
// (ver CLAUDE.md, restrição nº1 do repo). Parou de ter atividade => para
// de renovar => o token natural mente esfria e some sozinho depois de
// DURACAO_INATIVIDADE_MS, tanto no front (auth.js mostra "sessão
// expirada") quanto no back (qualquer rota via obterAdminSessao recusa).
//
// Só afeta obterAdminSessao (sessão de admin no painel) -- o token cru
// (verificarTokenPonto) continua sem expiração pra não quebrar
// debugPontoValidarToken, usado pelo checkout_bipagem.py (RobotOmie,
// fora deste repo) pra validar um token já obtido sem pedir login de
// novo -- não temos visibilidade de como esse processo local guarda/usa
// esse token, então mudar esse caminho é risco desnecessário pra esta
// tarefa.
const DURACAO_INATIVIDADE_MS = 5 * 60 * 1000;

function gerarTokenPonto(funcionario) {
  const payload = Buffer.from(JSON.stringify({ id: funcionario.id, nome: funcionario.nome, emissao: Date.now() })).toString('base64url');
...
// Proteção contra força bruta no PIN (4 dígitos, só 10 mil combinações) --
// depois de MAX_TENTATIVAS_LOGIN erradas SEGUIDAS, bloqueia esse
// funcionário por BLOQUEIO_LOGIN_SEGUNDOS antes de aceitar tentar de novo
// (ver debugPontoLogin, api/debug.js). Por funcionário, não por IP -- o
// PIN é o segredo de cada pessoa, e um dispositivo compartilhado (ex.: o
// tablet da Expedição) não pode travar todo mundo só porque uma pessoa
// errou o próprio PIN.
const MAX_TENTATIVAS_LOGIN = 5;
const BLOQUEIO_LOGIN_SEGUNDOS = 15 * 60; // 15 min
...
async function obterAdminSessao(token, db, pagina) {
  ...
  // Tokens sem `emissao` (formato antigo) ou emitidos/renovados há mais de
  // DURACAO_INATIVIDADE_MS são tratados como expirados -- força um novo
  // login em vez de ficarem válidos pra sempre ou por um prazo fixo.
  if (!funcionario.emissao || Date.now() - funcionario.emissao > DURACAO_INATIVIDADE_MS) {
    return { erro: 'Sessão expirada, faça login de novo.', status: 401 };
  }
...
module.exports = {
  ...
  DURACAO_INATIVIDADE_MS, MAX_TENTATIVAS_LOGIN, BLOQUEIO_LOGIN_SEGUNDOS,
};
