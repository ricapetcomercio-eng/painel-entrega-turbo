...
/**
 * Diagnóstico: lista campanhas do advertiser (sem paginar) — usado só pra
 * testar se a conta tem estrutura de campanhas (modo "Customizado") ou se
 * está no modo "Automático" (sem campanhas explícitas), que pode afetar
 * quais endpoints respondem dados.
 */
async function buscarCampanhas(conta, dataInicio, dataFim) {
  const accessToken = await getMLAccessToken(conta);
  const advertiserId = await buscarAdvertiserId(conta);
  const data = await mlAdsFetch(
    `/advertising/advertisers/${advertiserId}/product_ads/campaigns` +
      `?limit=10&offset=0&date_from=${dataInicio}&date_to=${dataFim}&metrics=clicks,prints,cost`,
    accessToken
  );
  return data;
}
...
/**
 * Diagnóstico: detalhe + métricas de UM anúncio específico (funciona
 * independente de modo automático/customizado, segundo a doc).
 */
async function buscarDetalheAnuncio(conta, itemId) {
  const accessToken = await getMLAccessToken(conta);
  return mlAdsFetch(`/advertising/product_ads/items/${itemId}`, accessToken);
}

/**
 * Diagnóstico bruto: faz um GET sem lançar erro, devolvendo status,
 * headers e corpo crus — usado só pra investigar o 404 vazio que as
 * chamadas normais de Ads estão dando mesmo com permissão e escopo certos.
 */
async function chamadaBruta(path, accessToken, headersExtra = {}) {
  const resp = await fetch(`https://api.mercadolibre.com${path}`, {
    headers: { Authorization: `Bearer ${accessToken}`, ...headersExtra },
  });
...
module.exports = { buscarAdvertiserId, buscarMetricasAnuncios, buscarCampanhas, buscarDetalheAnuncio, chamadaBruta };
