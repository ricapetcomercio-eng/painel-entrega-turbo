...
//   SHOPEE_<LOJA>_REFRESH_TOKEN   (fallback inicial; depois é renovado e salvo no Turso)
//
// Tabela: shopee_tokens (loja TEXT PRIMARY KEY, access_token, refresh_token, shop_id, expires_at)
// — migrado do Redis (chave shopee_token:<loja>), banco dedicado agora.
//
...
function getLojaConfig(loja) {
  ...
  // ficam salvos no Turso junto com o token, e são lidos de lá — evitando
...
async function buscarTokenSalvo(loja) {
  const db = getDb();
  const rs = await db.execute({
    sql: 'SELECT access_token, refresh_token, shop_id, expires_at FROM shopee_tokens WHERE loja = ?',
    args: [loja],
  });
...
/**
 * Salva/atualiza o token de uma loja. Exportado para uso direto em
 * api/shopee-callback.js (fluxo de autorização inicial), que precisa
 * gravar o primeiro token assim que a Shopee devolve o `code`.
 */
async function salvarTokenShopee(loja, dados) {
  const db = getDb();
  await db.execute({
    sql: `INSERT INTO shopee_tokens (loja, access_token, refresh_token, shop_id, expires_at)
          VALUES (?, ?, ?, ?, ?)
    ...
    args: [loja, dados.access_token, dados.refresh_token, dados.shop_id, dados.expires_at],
  });
}
...
async function trocarRefreshToken(loja) {
  ...
  const saved = await buscarTokenSalvo(loja);
  ...
  await salvarTokenShopee(loja, toSave);
...
module.exports = { getShopeeAccessToken, shopeeGet, LOJAS, fetchViaFixie, salvarTokenShopee };
