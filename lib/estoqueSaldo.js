...
// catalog.json vivia só em painel-estoque-adesivo (projeto Vercel separado,
// já desligado); agora que index.html/estoque.html daquele projeto viraram
// public/estoque.html etc. neste repo, o catálogo também mora aqui — lido
// direto do disco (mesmo deploy) em vez de um fetch HTTP pra outro domínio.
function lerCatalogo() {
  const conteudo = fs.readFileSync(path.join(__dirname, '../public/catalog.json'), 'utf8');
...
async function importarContagemFisica() {
  ...
  const respJsonbin = await fetch(`https://api.jsonbin.io/v3/b/${binId}/latest`, { headers: { 'X-Master-Key': apiKey } });
...
/**
 * Lê de volta o bloco mais recente já gravado na aba "Estoque Ricapet"
 * (via o doGet do mesmo Apps Script que enviarParaPlanilha/gravarNoSheets
 * já usa) e usa como novo valor de saldo — sempre sobrescreve, igual
 * importarContagemFisica. Complementa a importação do JSONBin: cobre
 * contagens que tenham sido feitas/editadas direto na planilha, sem passar
 * pelo painel-estoque-adesivo.
 */
async function importarSaldoDaPlanilha() {
  const url = process.env.GOOGLE_SHEETS_WEBAPP_URL;
  if (!url) throw new Error('Falta a variável de ambiente GOOGLE_SHEETS_WEBAPP_URL.');

  const resp = await fetch(url);
  if (!resp.ok) throw new Error(`Planilha respondeu ${resp.status} ao buscar o saldo.`);
  const json = await resp.json();
  if (!json.ok) throw new Error('doGet da planilha retornou erro: ' + json.error);

  const db = getDb();
  const agora = new Date().toISOString();
  let atualizados = 0;

  for (const linha of json.linhas || []) {
    const tamanho = linha.medida === 'única' ? '-' : linha.medida;
    const saldo = Number(linha.qtd) || 0;
    await db.execute({
      sql: `INSERT INTO estoque_saldo (produto, cor, tamanho, saldo, atualizado_em)
            VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(produto, cor, tamanho) DO UPDATE SET
              saldo = excluded.saldo,
              atualizado_em = excluded.atualizado_em`,
      args: [linha.produto, linha.cor, tamanho, saldo, agora],
    });
    atualizados++;
  }

  return { data_do_bloco: json.data, atualizados };
}
